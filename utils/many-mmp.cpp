	...
	while (fread(&headerData, sizeof headerData, 1, file) == 1) {
		...
		for (int c=0; c<C; ++c) {
			...
			for (int a=1; a<=atomCount; ++a) {
				...
				if (c==0 || atom.amplitude > atoms[a-1].amplitude) {
					atoms[a-1] = atom;
					channels[a-1] = c;
				}
			}
		}
		for (unsigned a=1; a<=atoms.size(); ++a) {
			const ReadAtom& atom = atoms[a-1];
			const int c = channels[a-1];
			if (atom.amplitude < 6) continue;
			if (atom.f < 11 || atom.f > 15) continue;
			fprintf(files[c], "%3d %2d %3d", epochNumber, c+1, a);
			for (unsigned i=0; i<(sizeof atom.params)/sizeof(float); ++i) {
				fprintf(files[c], " %18.15e", atom.params[i]);
			}
			fputc('\n', files[c]);
		}
	}
